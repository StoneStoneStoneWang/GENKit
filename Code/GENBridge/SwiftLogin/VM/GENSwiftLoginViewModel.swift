...
public struct GENSwiftLoginViewModel: GENViewModel {
    ...
    init(_ input: WLInput ,disposed: DisposeBag) {
        ...
        // 登录完成返回
        let logined: Driver<GENResult> = input.loginTaps.withLatestFrom(usernameAndVcode).flatMapLatest {
            
            switch GENCheckUsernameAndVCode($0.0, vcode: $0.1) {
            case .ok:
                
                return GENDictResp(GENApi.swiftLogin($0.0, code: $0.1))
            ...
            .flatMapLatest({ (username) in
                ...
                case .ok:
                    ...
                    let result: Observable<GENResult> = Observable<GENResult>.create({ (ob) -> Disposable in
                        GENVoidResp(GENApi.smsCode(username))
